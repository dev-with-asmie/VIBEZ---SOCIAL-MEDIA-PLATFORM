...
function Upload(){
    ...
    {/*const uploadWave = async ()=>{
      try {
         const formData = new FormData() 
         formData.append("media",backendMedia) 
         const result = await axios.post(`${serverUrl}/api/wave/upload`,formData,{withCredentials:true}) 
          dispatch(setWaveData([...waveData,result.data])) 
          setLoading(false) 
          navigate("/")
  ...
  try {
     const formData = new FormData() 
     formData.append("caption", caption)   // ✅ ADD THIS
     formData.append("media", backendMedia) 

     const result = await axios.post(
        `${serverUrl}/api/wave/upload`,
        formData,
   ...
   return (
    <div className='w-full h-[100vh] bg-gradient-to-b from-black to-gray-800  flex flex-col items-center'>
     ...
     <div className='w-[90%] max-w-[600px] h-[80px] bg-[white] rounded-full flex justify-around items-center gap-[10px]' >
          ...
          <div className={`${uploadType=="wave"?"bg-black text-white shadow-2xl shadow-black":""} w-[28%] h-[80%] flex justify-center items-center text-[19px] font-semibold hover:bg-black rounded-full hover:text-white cursor-pointer hover:shadow-2xl hover:shadow-black`} onClick={()=>setUploadType("wave")}>Wave</div>  
     </div>

     {!frontendMedia && <div className='w-[80%] max-w-[500px] h-[250px] bg-[#0e1316] border-gray-800 border-2 flex flex-col items-center justify-center gap-[8px] mt-[15vh] cursor-pointer rounded-2xl hover:bg-[#353a3d]'>
      <input type="file" accept={uploadType=="wave"?"video/*":""} hidden ref={mediaInput} onChange={handleMedia}/>
        <FiPlusSquare className='text-white w-[25px] h-[25px] cursor-pointer' />
         <div className='text-white text-[19px] font-semibold' onClick={()=>mediaInput.current.click()}>Upload {uploadType}</div>
     </div>}
     ...
     {frontendMedia && 
       <div className='w-[80%] max-w-[500px] h-[250px] flex flex-col items-center justify-center mt-[15vh]'> 
         {mediaType=="image" && 
          ...
          <div className='w-[80%] max-w-[500px] h-[250px] flex flex-col items-center justify-center mt-[5vh]'>
            ...
            {uploadType!="story" && <input key={uploadType} type='text' className='w-full border-b-gray-400 border-b-2 outline-none px-[10px] py-[5px] text-white mt-[20px]' placeholder='Write Caption...' onChange={(e)=>setCaption(e.target.value)} value={caption}/> }
       ...
       {frontendMedia && <button className='px-[10px] w-[50%] max-w-[400px] py-[5px] h-[50px] bg-[white] mt-[50px] cursor-pointer rounded-2xl' onClick={handleUpload}>{loading?<ClipLoader size={30} color='black'/>:`Upload ${uploadType}`}</button>} 
